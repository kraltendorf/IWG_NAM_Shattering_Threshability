...
dirs <- dirs[grepl("rachis_breaks_mean|floret_score_mean|threshability", dirs)]
...
emmeans_env <- data.frame(matrix(NA, nrow = 1295, ncol = 3))
emmeans_all <- list()
...
for (i in 1:length(emmeans_all)){
  emmeans_all[[i]] <- emmeans_all[[i]] %>% select(all_traits)
