...
### make a wide table per steve's request with combined QTL ####
combined <- lod_intervals_final %>% 
  filter(trait %in% c("floret_score_mean", "rachis_breaks_mean", "threshability")) %>%
  filter(famID == "combined") %>%
  select(-r2, -mu, -alpha, -gamma)

combined1 <- combined %>% group_by(trait) %>% select(-left, -right) %>% pivot_wider(names_from = c(env, year), values_from = c(center, max_lod, locus))
combined2 <- combined1 %>% dplyr::select(trait, lg, map, 
                                         center_stp_2017, locus_stp_2017, max_lod_stp_2017,
                                         center_stp_2018, locus_stp_2018, max_lod_stp_2018,
                                         center_tli_2017, locus_tli_2017, max_lod_tli_2017)

write.csv(combined2, paste(dir, folder, "/output/combined_ed.csv", sep = ""), row.names = FALSE)
