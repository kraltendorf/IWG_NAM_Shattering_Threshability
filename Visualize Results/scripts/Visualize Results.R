...
View(test)
domestication_genes <- read.csv("/Users/kayla.altendorf/OneDrive - USDA/Publications/IWG NAM Shattering and Threshability/Scripts for Github/Visualize Results/data/candidate_genes.csv", header = T) %>% 
...
gwas_hits <- read.csv("/Users/kayla.altendorf/OneDrive - USDA/Publications/IWG NAM Domestication Traits/Scripts for Github/GWAS/output/final_gwas_all.csv", header = T) %>%
  dplyr::select(Chromosome, Position, trait, loc, year)%>% 
  mutate(loc_year = paste(loc, substr(year, 3, 4), sep = "")) %>%
  dplyr::select(-loc, -year) %>%
  tidyr::pivot_wider(names_from = loc_year, values_from = loc_year) %>%
  tidyr::unite("envs", STP17:TLI18, sep = ", ", na.rm = T)
...
gwas_hits_formatted <- read.csv("/Users/kayla.altendorf/OneDrive - USDA/Publications/IWG NAM Domestication Traits/Scripts for Github/Visualize Results/output/gwas_hits_formatted_ed.csv")
...
combined_qtl
adjuster <- data.frame(marker_name = c("w", "w", "w", "w", "w"), 
                       cm_pos = c(400, 310, 63.489, 47.654, 28.677), 
                       group = c("LG02", "LG08", "LG05", "LG05", "LG03"), 
                       bp_pos = rep(NA, 5),
                       chr = c(2, 8, 5, 5, 3), 
                       display = rep("adjuster", 5),
                       bp_min = c(109865, 1162650, 1153145, 1153145, 319865), 
                       bp_max = c(430937086, 431292282, 434141578, 434141578, 513591594), 
                       cm_min = rep(0, 5), 
                       cm_max = c(143.98, 180.245, 146.36, 146.36, 129.578),
                       map = rep("genetic", 5),
                       position = c(380, 310, 433.79, 325.59, 238.56))

# add the adjuster markers to the map 
all_data2 <- rbind(all_data2, adjuster)
all_data2 %>% filter(group == "LG03")
combined_qtldf
...
lod_intervals_ed <- lod_intervals %>% 
  ...
  filter(trait %in% c("rachis_breaks_mean","floret_score_mean", "threshability"))
...
combined_qtl <- combined_qtl %>% filter(trait %in% c("rachis_breaks_mean","floret_score_mean", "threshability")) #%>%
